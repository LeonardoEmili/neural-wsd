...
class OMSTIReader(RaganatoReader):
    ...
    def read(
        ...
        **kwargs,
        ...
        omsti_data = super(OMSTIReader, cls).read(path_dict["omsti"], key_path, split=split)
        if kwargs.get("merge_with_semcor", False):
            # Test with SemCor + OMSTI
            semcor_data = super(OMSTIReader, cls).read(path_dict["semcor"], key_path, split=split)
